...
sealed class Result<T> {
  ...
  R fold<R>({required R Function(T) ok, required R Function(Failure) err}) => switch (this) {
...
extension ResultX<T> on Result<T> {
  void onOk(void Function(T) f) {
    if (this is Ok<T>) f((this as Ok<T>).value);
  }

  void onErr(void Function(Failure) f) {
    if (this is Err<T>) f((this as Err<T>).error);
  }
...
class NetworkFailure extends Failure {
  ...
  const NetworkFailure(super.message, {this.code, this.method, this.url, this.duration, this.tokenExpired = false, this.isTimeout = false, this.isConnection = false});
  ...
  // --- Config knobs (can be toggled via NetworkManager at init) ---
  ...
  // --- Builders ---
  static NetworkFailure fromResponse({required String method, required String url, required int? code, required String message, required Duration duration, bool tokenExpired = false}) {
  ...
  static NetworkFailure fromException(Object error, {String? fallbackMessage}) {
    ...
    try {
      ...
      if (typeName.contains('DioException')) {
        ...
        code = (error as dynamic).response?.statusCode as int?;
    ...
    return NetworkFailure(_short(msg, max: 140), code: code, method: method, url: url, duration: null, tokenExpired: expired, isTimeout: timeout, isConnection: connection);
  }
}
